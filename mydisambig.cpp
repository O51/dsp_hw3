            ...
            for(int i=1;i<Input.size();i++)
            {
                ...
                for(int veter=0;veter<possible[i].size();veter++)
                {
                    ...
                    for(int cf=0;cf<possible[i-1].size();cf++)
                    {
                        ...
                        else if(model.count(possible[i][veter]))connect[possible[i-1][cf]+possible[i][veter]]=model[possible[i][veter]]-30;
                        else connect[possible[i-1][cf]+possible[i][veter]]=model["<unk>"]-300;
